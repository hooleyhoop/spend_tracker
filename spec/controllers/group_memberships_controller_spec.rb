...
describe GroupMembershipsController do
  ...
  def mock_group_membership(stubs={})
    (@mock_group_membership ||= mock_model(GroupMembership).as_null_object).tap do |group_membership|
      group_membership.stub(stubs) unless stubs.empty?
    end
  end
  ...
  describe "DELETE destroy" do
    ...
    it "redirects to the group_memberships list" do
      GroupMembership.stub(:find) { mock_group_membership }
      delete :destroy, :id => "1"
